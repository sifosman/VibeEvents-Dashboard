...
import { ChevronDown, Filter, Briefcase } from "lucide-react";
...
export default function ServiceProviderSearch({ categoryId }: ServiceProviderSearchProps) {
  ...
  const { data: serviceProviders, isLoading } = useQuery<Vendor[]>({
    queryKey: ['/api/service-providers/search', {
      categoryId,
    ...
    queryFn: async () => {
      ...
      const res = await fetch(`/api/service-providers/search?${params}`);
    ...
    <>
      ...
      <div className="container-custom py-8">
        <div className="mb-6">
          ...
          <p className="text-muted-foreground">Find the perfect service provider for your event</p>
        </div>
        ...
        <div>
          <h2 className="text-xl font-semibold mb-4">
            {serviceProviders ? `${serviceProviders.length} service providers found` : 'Searching service providers...'}
          </h2>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {serviceProviders.map((serviceProvider) => (
                <VendorCard key={serviceProvider.id} vendor={serviceProvider} />
              ))}
            </div>
          ) : (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">Try adjusting your search filters to find more service providers.</p>
            </div>
          )}
        </div>
      </div>
