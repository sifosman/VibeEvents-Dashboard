...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Public config for client-only dashboard (Supabase URL and anon key)
  app.get('/api/dashboard-config', async (_req: Request, res: Response) => {
    try {
      const supabaseUrl = process.env.SUPABASE_URL || '';
      const supabaseAnonKey = process.env.SUPABASE_ANON_KEY || '';
      return res.status(200).json({ supabaseUrl, supabaseAnonKey, bucket: 'Funxions' });
    } catch (error) {
      return res.status(500).json({ message: 'Config not available' });
    }
  });
      ...
      const sampleVendors = [
        {
          ...
          dietaryOptions: ["Vegetarian", "Vegan", "Halaal", "Gluten Free"],
          cuisineTypes: ["South African", "International", "Contemporary"],
          calendarView: true,
          cataloguePages: 6
        },
        {
